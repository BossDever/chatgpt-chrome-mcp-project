...
async function main() {
  ...
  try {
    ...
    assert.equal(names.length, 28, "unexpected MCP tool count");
    assert.equal(chatgptCdpToolCount + chromeCdpToolCount, 14, "unexpected CDP tool count");
    ...
    assert(names.includes("chatgpt_cdp_save_generated_image"), "missing chatgpt_cdp_save_generated_image");
